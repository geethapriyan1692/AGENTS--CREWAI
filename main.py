...
def startupLoading():
    try:
        ...
        crew_file = "/code/agents/crew.py"  # crew.py file path
        filename = r"/code/agents/AgentFunction.py" # File where all code generated by generator_task is present or code generated must be updated in this file.
        ...
        # Agents.yaml file
        agent_file_path = "/code/agents/agents.yaml"
        ...
        # Tasks.yaml file
        task_file_path = "/code/agents/tasks.yaml"
        # Check if the file exists
        ...
        # Increment the count if the agent_name is the same
            ...
            if config_type == 'custom_logic':
                ...
                filename = "/code/agents/agents.yaml"
                # Check if the file exists, if not, create it
                if not os.path.exists(filename):
                    with open(filename, 'a', encoding='utf-8') as f:
                        pass  # Create an empty file
                ...
                filename = "/code/agents/tasks.yaml"
        ...
        for i in range(0,agents_count):
            ...
            if method_name + "_generator" in agents_data.keys():
                ...
                if(len(rows)>0):
                    ...
                    file_paths = [
                        "/code/agents/agents.yaml",
                        "/code/agents/tasks.yaml",
                    ]
                    ...
                    code_file="/code/agents/AgentFunction.py"
                                # Read original code
...
async def conversationAgent(input: str = Form(...), 
    ...
    async def generate():
            ...
            async def on_llm_end(self, response, **kwargs):
                ...
                async def submit_order():
                    ...
                    if "comprehensive summary" in llm_response.lower():
                        payload = {
                            "prompt": """You are a data extraction and JSON formatting expert. Your task is to extract specific information from a given LLM Response and structure it into a valid JSON object.
            
                        The text you will be processing is a response from another Large Language Model. You must identify key entities and their corresponding values, and then present this data as a single JSON object.
            
                        **Constraints:**
                        - The final output MUST be a valid JSON object.
                        - DO NOT include any text, explanations, or prose outside of the JSON object.
                        - If a value is not present in the text, you should use `null` for that key.
                        -  Do not generate json placeholder, Return only the json starting and ending with curly braces.
            
                        Always the output response json should like
                        **
                        "Customer Name": ...,
                        "Address Line 1": ...,
                        "City": ...,
                        ...
                        **
                        ...
                        And here is the LLM Response: {llm_res}
            
                        """.format(llm_res = llm_response),
                            "model": "llama3.3",
                            "stream": False
                        }
                        ...
                        inbound_request = json.loads(response_json.get("response", ""))
                        ...
                        try:
                            ...
                            if notify_flag:
                                ...
                                notify_response_dict = notify(session_id, notify_payload)    
        ...
        llm = Ollama(model=config_loader.get('conversation_model'), base_url="http://prod-llm.excelacomcloud.net:11434", callbacks=[callback])
